...
public:
    ...
    int calculate_hops_travelled(int src_vault, int dst_vault) {
      ...
      return src_vault==dst_vault? 0 : channel_change_latency*per_hop_overhead;
    }

    int calculate_broadcast_hops_travelled(int src_vault) {
      assert(src_vault >= 0);
      return calculate_hops_travelled(0,1);
    }
    ...
    std::map<std::string, Type> name_to_type = {
      {"ChRaBaRoCo", Type::ChRaBaRoCo},
      {"RoBaRaCoCh", Type::RoBaRaCoCh}};
    ...
    private:
      ...
      class SubscriptionTable{
        ...
        Memory<HBM, Controller>* mem_ptr = nullptr;
        ...
        SubscriptionTable(){}
        SubscriptionTable(size_t size, size_t ways, size_t receiving_buffer_size, Memory<HBM, Controller>* mem_ptr):subscription_table_size(size),subscription_table_ways(ways),receiving_buffer_size(receiving_buffer_size), mem_ptr(mem_ptr){initialize();} // Only set from table size
    ...
    public:
      explicit SubscriptionPrefetcherSet(int controllers, Memory<HBM, Controller>* mem_ptr):controllers(controllers),mem_ptr(mem_ptr) {
      ...
      long find_mirror_address(int vault, long addr) {
        ...
        victim_vec[int(HBM::Level::Channel)] = vault;
      ...
      void access_address(Request& req) {
        ...
        int original_vault_id = req.addr_vec[int(HBM::Level::Channel)];
        ...
        if(subscription_tables[original_vault_id].is_subscribed(addr) || subscription_tables[original_vault_id].is_pending_removal(addr)) {
          ...
          req.addr_vec[int(HBM::Level::Channel)] = val_vault_id;
        }
        total_memory_accesses++;
      }
      void update_count_table_and_submit_subscription(const Request& req) {
        int req_vault_id = req.coreid;
        int val_vault_id = req.addr_vec[int(HBM::Level::Channel)];
        long addr = req.addr;
        pre_process_addr(addr);
          ...
          prefetcher_set(ctrls.size(), this)
    {
        ...
        cout << "Number of cores in HBM Memory: " << configs.get_core_num() << endl;
        cout << "Read latency of HBM Memory: " << ctrls[0]->channel->spec->read_latency << " cycles" << endl;
        cout << "Write latency of HBM Memory: " << ctrls[0]->channel->spec->write_latency << " cycles" << endl;
 ...
 //       cout << "maxblock_entry.flit_num_bits: " << spec->maxblock_entry.flit_num_bits << " tx_bits: " << tx_bits << " max_block_col_bits: " << max_block_col_bits << endl;
        ...
        for (auto ctrl : ctrls) {
          ...
          ctrl->attach_parent_update_latency_function(std::bind(&Memory<HBM, Controller>::SubscriptionPrefetcherSet::update_latency, &(this->prefetcher_set), std::placeholders::_1));
    ...
    long address_vector_to_address(const vector<int>& addr_vec) {
      long addr = 0;
      long vault = addr_vec[int(HBM::Level::Channel)];
      long bank_group = addr_vec[int(HBM::Level::BankGroup)];
      long rank = addr_vec[int(HBM::Level::Rank)];
...
// Types are :         ChRaBaRoCo,  RoBaRaCoCh,
      ...
      switch(int(type)) {
        case int(Type::ChRaBaRoCo): {
          ...
          addr |= bank;
          addr <<= addr_bits[int(HBM::Level::Row)];
          addr |= row;
    ...
    bool send(Request req)
    {
      ...
      //  adddd_vec =  address_to_address_vector(addr);
       // cout << "original vault of address " << addr << " after clearing.. = " << adddd_vec[int(HBM::Level::Channel)] << endl;
        ...
        if (subscription_prefetcher_type != SubscriptionPrefetcherType::None) {
         ...
         int subscribed_vault = req.addr_vec[int(HBM::Level::Channel)];
      ...
      { 
            ...
            if(req.coreid >= 0 && req.coreid < 256) {
              ...
              int simplified_hops = calculate_hops_travelled(req.coreid, req.addr_vec[int(HBM::Level::Channel)])/per_hop_overhead;
              hops_distribution[simplified_hops]++;
              assert(address_distribution[req.coreid][req.addr_vec[int(HBM::Level::Channel)]] >= 0);
              address_distribution[req.coreid][req.addr_vec[int(HBM::Level::Channel)]]++;
              assert(address_distribution[req.coreid][req.addr_vec[int(HBM::Level::Channel)]] >= 0);
              if (req.type == Request::Type::WRITE){
                hops_distribution_w[simplified_hops]++;
                address_distribution_w[req.coreid][req.addr_vec[int(HBM::Level::Channel)]]++;
              } else if (req.type == Request::Type::READ) {
                hops_distribution_r[simplified_hops]++;
                address_distribution_r[req.coreid][req.addr_vec[int(HBM::Level::Channel)]]++;
              } else{
                hops_distribution_o[simplified_hops]++;
                address_distribution_o[req.coreid][req.addr_vec[int(HBM::Level::Channel)]]++;
              }
            }
            else
              cerr << "HBM MEMORY: INVALID CORE ID: " << req.coreid << "endl";
            ...
            if(get_memory_addresses){
              if (profile_this_epoch){
                ...
                memory_addresses << req.addr_vec[int(HBM::Level::Channel)] << "," << req.addr_vec[int(HBM::Level::BankGroup)] << ","
                                 << req.addr_vec[int(HBM::Level::Bank)] << ","  << req.addr_vec[int(HBM::Level::Row)]       << ","
                                 << req.addr_vec[int(HBM::Level::Column)] << "\n";
